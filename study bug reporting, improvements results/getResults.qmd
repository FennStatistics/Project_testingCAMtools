...
suppressMessages(read_file('jatos_results_data_20240430143549.txt') %>%
  # ... split it into lines ...
  str_split('\n') %>% first() %>%
  # ... filter empty rows ...
...
dat_pictures$Base64Image <- str_remove_all(string = dat_pictures$Base64Image, pattern = "data:image/png;base64,")
...
for(i in 1:nrow(dat_pictures)){
  if(!is.na(dat_pictures$Base64Image[i])){
    ...
    writeBin(binary_data, paste0(dat_pictures$ID[i], "_screenshot.png"))
  }
}
```
